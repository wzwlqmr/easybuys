...
var itemListOver = {
    ...
    bindEvents: function(){
        var self = this;
        //******注释代码,解决所有地址都是旧地址  未绑定事件bug  begin *****
        //if( !this.dom.find(".ui-switchable-panel").length ){
        //    return;
        //}
        //******注释代码,解决所有地址都是旧地址  未绑定事件bug  end  *****
        this.dom.delegate('.consignee-item','click',function(event){
            var tempName = event.target.nodeName;
            var tempHtml = $(event.target).html();
            if(tempName == "DIV" || tempName == "SPAN"){
		        //选中的地址不允许删除，由于此时已经选中了当前地址，需要恢复之前选中地址的删除按钮
		        var itemSelectedAddressDiv = self.dom.find(".item-selected");
		        itemSelectedAddressDiv.each(function(){
	               var $this = $(this);
	                   $this.next().next().find(".del-consignee").removeClass("hide");
                });
                //移除之前选中地址的选中样式
                self.dom.find(".consignee-item").removeClass("item-selected");
				//移除之前选中地址的选中样式,错误地址有被选中的情况
                self.dom.find(".consignee-item-disable").removeClass("item-selected");
                    $(this).addClass("item-selected");
                    //当前选中地址不能删除,设置"删除"不可见
                    $(this).next().next().find(".del-consignee").addClass("hide");
                    
                    $("#selfPickSiteName").removeClass("item-selected");
                    tab_save_Consignee();
            }
        });
        this.dom.delegate('.op-btns', 'click', function(event){
            var tempName = event.target.nodeName;
            var tempHtml = $(event.target).html();
            if(tempHtml=="编辑"){
                openEditConsigneeDialog($(this).attr('consigneeId'));
            }else if(tempHtml=="删除"){
                deleteConsigneeDialog($(this).attr('consigneeId'));
            }else if(tempHtml=="设为默认地址"){
                setAllDefaultAddress($(this).attr('consigneeId'));
		        //选中的地址不允许删除，由于此时已经选中了当前地址，需要恢复之前地址的删除按钮
                var itemSelectedAddressDiv = self.dom.find(".item-selected");
		        itemSelectedAddressDiv.each(function(){
		               var $this = $(this);
		                   $this.next().next().find(".del-consignee").removeClass("hide");
	            ...
	            //移除之前选中地址的选中样式
                self.dom.find(".consignee-item").removeClass("item-selected");
				//移除之前选中地址的选中样式,错误地址也有被选中的情况
                self.dom.find(".consignee-item-disable").removeClass("item-selected");
                //yanwenqi 自提地址项目
                $("#selfPickSiteName").removeClass("item-selected");
                $(this).prev().prev().addClass("item-selected");
                //当前选中地址不能删除
                $(this).find(".del-consignee").addClass("hide");
                tab_save_Consignee();
            }
        });
        //****** 2017-03-08地址服务升级  "绑定地址服务升级信息中的操作事件" begin ******
        this.dom.delegate('.upgrade-ops', 'click', function(event){
            var tempName = event.target.nodeName;
            var tempHtml = $(event.target).html();
            if(tempHtml=="编辑"){
            	openEditConsigneeDialog($(this).attr('consigneeId'));
            }else if(tempHtml=="属于"){
            	window.save_belong_area($(this).attr('consigneeId'));
            }
        });
        //****** 2017-03-08地址服务升级  "绑定地址服务升级信息中的操作事件" end ******
    }
};
