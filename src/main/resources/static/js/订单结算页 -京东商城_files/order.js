...
function resetFreightDetailWeight(){
	...
	$("#xzsamweight-detail").html("");

    var allXuZhongWeightSamGenShow = $("#allXuZhongWeightSamGenShow").val();
    if(allXuZhongWeightSamGenShow != undefined && allXuZhongWeightSamGenShow != ""){

        var html = "（总重";
        html += allXuZhongWeightSamGenShow;

        var overXuZhongWeightSamGen = $("#overXuZhongWeightSamGen").val();
        if(overXuZhongWeightSamGen != undefined && overXuZhongWeightSamGen != ""){
            html += "，超出重量" + overXuZhongWeightSamGen + "）";
        } else {
            html += "）";
        }

        $("#xzsamweight-detail").html(html);
    }
    
    $("#xzsamsxweight-detail").html("");

    var allXuZhongWeightSamSXShow = $("#allXuZhongWeightSamSXShow").val();
    if(allXuZhongWeightSamSXShow != undefined && allXuZhongWeightSamSXShow != ""){

        var html = "（总重";
        html += allXuZhongWeightSamSXShow;

        var overXuZhongWeightSamSX = $("#overXuZhongWeightSamSX").val();
        if(overXuZhongWeightSamSX != undefined && overXuZhongWeightSamSX != ""){
            html += "，超出重量" + overXuZhongWeightSamSX + "）";
        } else {
            html += "）";
        }

        $("#xzsamsxweight-detail").html(html);
    }
    
	
	$("#scfeight-detail").html("");
	var allXuZhongWeightSCFShow = $("#allXuZhongWeightSCFShow").val();
	 if(allXuZhongWeightSCFShow != undefined && allXuZhongWeightSCFShow != ""){

	        var html = "（总重";
	        html += allXuZhongWeightSCFShow;

	        var overXuZhongWeightSCF = $("#overXuZhongWeightSCF").val();
	        if(overXuZhongWeightSCF != undefined && overXuZhongWeightSCF != ""){
	            html += "，超出重量" + overXuZhongWeightSCF + "）";
	        } else {
	            html += "）";
	        }

	        $("#scfeight-detail").html(html);
	    }
...
function consigneeChangeTip(){
	if($("#downConsigneeChangeTip").val() != 1){
		var consigneeid = $(".consignee-item.item-selected").attr("consigneeid");
		var select_li = $("#consignee_index_"+consigneeid);
		var allLi = $(".J_toUpgrade.showChangeJingAddres");
		if(typeof allLi !=='undefined' && allLi.length >0){
			for(var i=0;i<allLi.length;i++){
				if($.browser.msie){
					allLi[i].removeNode(true);
				}else{
					allLi[i].remove();
				}
		...
		if($("#showChangeJingAddressFlag").val() =="true"){
			 var sj_li= "<li class='J_toUpgrade showChangeJingAddres'  id='showChangeJingAddres_"+consigneeid+"' parent_id='consignee_index'>";
		     sj_li = sj_li+"<div class='upgrade-item'><i class='arrow'></i>";
		     sj_li = sj_li+"<div class='upgrade-cont'>"
		     sj_li = sj_li+"<i></i>由于地址库升级，已为您匹配新的地址</div></div></li>"
		     select_li.after(sj_li);
		     $('#consignee-addr .consignee-cont').css({'height':'84px'});
		     $('#consignee-addr .ui-scrollbar-wrap').css({'height':'84px'});
		}else{
			var disableId = $(".consignee-item-disable.item-selected").attr("consigneeid");
...
function flushPlusInfo(type){
	if($("#totalPriceVender").val()=="0"|| typeof $("#totalPriceVender").val() =='undefined'){
		...
		var actionUrl = OrderAppConfig.DynamicDomain + "/order/checkPlusQualification.action";
		 jQuery.ajax({
		     type : "POST",
		     dataType : "json",
		     url : actionUrl,
		     ...
		     success : function(data) {
		        if (isUserNotLogin(data)) {
		          ...
		          return;
		        	...
		        	if(plusInfo != null && plusInfo.plusInfos.length>0 && data == 1){
		    			var plusProductList;
		    			var contentMoney;
		    			$("#checkPlusQualification").val(data);
		    			if(type=="1"){
		    		    ...
		    		    }else if($('#plusStatus').val()=="1" && 0<$('#totalPriceVender').val()<149){
...
function checkPaymentPasswordSafe(type, giftCardType) {
  ...
  jQuery.ajax({
    ...
    function flushOrderFreightDetail(orderPrice, typeFlag) {
    	...
    	//处理同城速配文案
    	var fsxtcbase = $("#fsx_base_tc").html();
    	var sxtcbase = $("#sx_base_tc").html();
    	var sxtcbaseshds = $("#sx_base_tc_shds").html();
    	var zyTcJsd = $("#zyTcJsd").val();
    	if($("#gsd_shipment_item").hasClass("curr") && zyTcJsd!=""){
    		if(fsxtcbase != null){
    			$("#fsx_base_tc").html(fsxtcbase.replace('基础运费', zyTcJsd));
    		}
    		if(sxtcbase !=null){
    			$("#sx_base_tc").html(sxtcbase.replace('基础运费', zyTcJsd));
    		}
    		if(sxtcbaseshds !=null){
    			$("#sx_base_tc_shds").html(sxtcbaseshds.replace('基础运费', zyTcJsd));
    		}
    	}else{
    		if(fsxtcbase != null){
    			$("#fsx_base_tc").html(fsxtcbase.replace(zyTcJsd,'基础运费'));
    		}
    		if(sxtcbase !=null){
    			$("#sx_base_tc").html(sxtcbase.replace(zyTcJsd,'基础运费'));
    		}
    		if(sxtcbaseshds !=null){
    			$("#sx_base_tc_shds").html(sxtcbaseshds.replace(zyTcJsd,'基础运费'));
    		}
    	}
    	// 初始化运费明细
        ...
        $("#xzsamsxweight-detail").hide();
        $("#xzsamweight-detail").hide();
        $("#scfeight-detail").hide();
       ...
       //全球售非图书基础运费
        ...
        var hasgsdFreight = 0;
        //先隐藏
        var arr1= $(".sop-jzd-freight_class");
        var arr2= $(".sop-djzd-freight_calss");
        var arr3= $(".sop-jzdsx-freight_calss");
        var arr4= $(".sop-jzdsxdj-freight_class");
        arr1.each(function(){
            $(this).hide();
        });
        arr2.each(function(){
            ...
            $("#tooltip-box06 b.free-freight").each(function() {
                ...
                if (prop != 0) { // pop
                    if (freightSop == "0.00" || freightSop == "0" || freightSop == "") { // 原有判断逻辑
                    	...
                    	//scf续重运费
                    	var scfXuZhongWeight = toNumber(orderPrice.detail["202"]);
                    	if(scfXuZhongWeight > 0 && prop == "10214981"){
                    		freightSop = freightSop -scfXuZhongWeight;
                    		$("#scf-xz-freight").html("￥" + Number(scfXuZhongWeight).toFixed(2));
                    		$("#scf-xz-freight").parent().show();
                    		$("#scfeight-detail").show();
                    		$curFreeFreightBlock.hide();
                    	}
                        // 展示基础运费
...
//            var hasJdExtFreight = 0;
            ...
            //处理山姆
            if(prop == "603837" && $("#samfreight").val() == "0"){
            	//山姆普通基础运费
            	var samBasefreight = toNumber(orderPrice.detail["26"]);
            	//山姆普通续重运费
            	var samXzfreight = toNumber(orderPrice.detail["11"]);
            	//山姆生鲜基础运费
            	var samSxBasefreight = toNumber(orderPrice.detail["230"]);
            	//山姆生鲜续重运费
            	var samSxXzfreight = toNumber(orderPrice.detail["232"]);
            	
            	if((samBasefreight >0 || samXzfreight>0) && (samSxBasefreight >0 || samSxXzfreight>0)){
            		$("#sam_sx-freight-split-line").show();
            	}else{
            		$("#sam_sx-freight-split-line").hide();
            	}
            	if(samBasefreight >0 || samXzfreight>0 || samSxBasefreight >0 || samSxXzfreight>0){
            		$("#sam_free_freight").parent().hide();
            	}else{
            		$("#sam_free_freight").parent().show();
            	}
            	
            	if(samBasefreight >0 || samXzfreight>0){
            		$("#sam_normal-freight-container").show();
            		if(samBasefreight >0){
            			$("#sam_normal-freight-container .sam_base-freight").html("￥"+samBasefreight);
            			$("#sam_normal-freight-container .sam_base-freight").parent().show();
            		}else{
            			$("#sam_normal-freight-container .sam_base-freight").parent().hide();
            		}
            		if(samXzfreight>0){
            			$("#sam_normal-freight-container .sam_xz-freight").html("￥"+samXzfreight);
            			$("#sam_normal-freight-container .sam_xz-freight").parent().show();
            		}else{
            			$("#sam_normal-freight-container .sam_xz-freight").parent().hide();
            		}
            	}else{
            		$("#sam_normal-freight-container").hide();
            	}
            	if(samSxBasefreight >0 || samSxXzfreight>0){
            		$("#sam_sx-freight-container").show();
            		if(samSxBasefreight >0){
            			$("#sam_sx-freight-container .sam_sx_base-freight").html("￥"+samSxBasefreight);
            			$("#sam_sx-freight-container .sam_sx_base-freight").parent().show();
            		}else{
            			$("#sam_sx-freight-container .sam_sx_base-freight").parent().hide();
            		}
            		if(samSxXzfreight >0){
            			$("#sam_sx-freight-container .sam_sx_xz-freight").html("￥"+samSxXzfreight);
            			$("#sam_sx-freight-container .sam_sx_xz-freight").parent().show();
            		}else{
            			$("#sam_sx-freight-container .sam_sx_xz-freight").parent().hide();
            		}
            	}else{
            		$("#sam_sx-freight-container").hide();
            	}
            	if((samBasefreight >0 || samXzfreight>0) && (samSxBasefreight >0 || samSxXzfreight>0)){
            		$("#sam_sx-freight-split-line").show();
            	}else{
            	...
            	if(samXzfreight > 0){
                    $("#xzsamweight-detail").show();
            	}else{
            		$("#xzsamweight-detail").hide();
            	}
            	if(samSxXzfreight > 0){
            		$("#xzsamsxweight-detail").show();
            	}else{
            		$("#xzsamsxweight-detail").hide();
            	}
            }
            //处理pop京准达运费
            var sopJzdFreight = orderPrice.sopJzdFreight;
            if(sopJzdFreight !=null){
            	for(var key in sopJzdFreight){
            		//普通京准达
            		var popjzd;
            		//普通大件京准达
            		var popdjjzd;
            		...
            		//生鲜大件京准达
            		var popsxdjjzd;
                	var sopjzdfee = sopJzdFreight[key];
                	//var sopSxjzdfee = sopJzdFreight[key+"SOPSX"];
                	if(sopjzdfee !=null){
                		popjzd = sopjzdfee[101];
            			popdjjzd = sopjzdfee[102];
            			popsxjzd = sopjzdfee[121];
            			popsxdjjzd = sopjzdfee[122];
                	}
                	if(Number(popjzd).toFixed(2)>0){
            			$("#sop-jzd-freight_"+key).html("￥" + Number(popjzd).toFixed(2));
            			$("#sop-jzd-freight_"+key).parent().show();
            			$(".sop-free-freight_"+key).hide();
            		}else{
            			$("#sop-jzd-freight_"+key).parent().hide();
            		}
            		if( Number(popdjjzd).toFixed(2)>0){
            			$("#sop-djzd-freight_"+key).html("￥" + Number(popdjjzd).toFixed(2));
            			$("#sop-djzd-freight_"+key).parent().show();
            			$(".sop-free-freight_"+key).hide();
            		}else{
            			$("#sop-djzd-freight_"+key).parent().hide();
            		}
            		if(Number(popsxjzd).toFixed(2)>0){
            			$("#sop-jzdsx-freight_"+key.replace("SOPSX" ,"")).html("￥" + Number(popsxjzd).toFixed(2));
            			$("#sop-jzdsx-freight_"+key.replace("SOPSX" ,"")).parent().show();
            			$(".sop-free-freight_"+key.replace("SOPSX" ,"")).hide();
            		}else{
            			$("#sop-jzdsx-freight_"+key.replace("SOPSX" ,"")).parent().hide();
            		}
            		if(Number(popsxdjjzd).toFixed(2)>0){
            			$("#sop-jzdsxdj-freight_"+key.replace("SOPSX" ,"")).html("￥" + Number(popsxdjjzd).toFixed(2));
            			$("#sop-jzdsxdj-freight_"+key.replace("SOPSX" ,"")).parent().show();
            			$(".sop-free-freight_"+key.replace("SOPSX" ,"")).hide();
            		}else{
            			$("#sop-jzdsxdj-freight_"+key.replace("SOPSX" ,"")).parent().hide();
            		}
                }
            }
        }
...
function fulshPlusInfoByFreight(orderPrice) {
    ...
    var FreeFreight_coupon = 0;
    if(typeof $("#hiddenFreeFreight_coupon").val() !=='undefined' && $("#hiddenFreeFreight_coupon").val() !=""){
    	FreeFreight_coupon = $("#hiddenFreeFreight_coupon").val();
    }
...
function choosePresaleType2(thisObj) {
  if ($(thisObj).prop("id") == "EarnestPayRadio") {
    ...
    $(".presale-con .pay-chk").css("display", "none");
    $("#presaleEarnest").attr("checked", true);
    if('2' === $('#virtualdiv').attr('data-forbidtype') || $('#virtualdiv').attr('redpaket-forbidtype') == "false"){
        query_coupons_vertual(true,true);
        $("#virtualdiv").css('pointer-events','auto');
        $('#virtualdiv').find('i').css('display','block');
        $('.order-presale').css('display','none');
            ...
            if (dataResult.sopNameMap[key] != null && dataResult.sopNameMap[key] != "undefined") {
              if (dataResult.sopNameMap[key].length > 16)
                ...
                if(sopName.indexOf("、") != -1){
                	...
                	if(key=="10214981" && $("#scfVenderName").val() !=null && $("#scfVenderName").val()!=""){
                		$(this).html($("#scfVenderName").val());
                	}else{
                		$(this).html(dataResult.sopNameMap[key]);
                	}
                }
...
}window.showCommitRule= showCommitRule;